# ============================================================
# CloudPulse - High availability (matches data_encryption edge security)
# S3 CRR to eu-west-3, DynamoDB replica, ASG min/desired 2
# Internal ALB, CloudFront VPC Origin, origin secret header, WAF
# us-east-1: certificate.tf (ACM). Secondary: eu-west-3 for S3 replica.
# ============================================================
terraform {
  required_providers {
    aws = {
...
resource "aws_cloudwatch_metric_alarm" "dynamodb_replication_lag" {
  ...
  statistic           = "Average"
  threshold           = "10000"
  alarm_description   = "DynamoDB global table replication lag"
  alarm_actions       = []
